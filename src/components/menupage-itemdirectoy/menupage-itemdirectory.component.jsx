...
import MenuPageItems from "../menupage-items/menupage-items.component";

import './menupage-itemdirectory.styles.scss';


class MenuPageItemDirectory extends React.Component {
    constructor() {
        super();
    ...
    render() {
        return (
            <div className='menupageitem-directory'>
               {
                   this.state.MENU_DATA.map(({name, imageUrl, id, ingredients, price}) => (
                       <MenuPageItems key={id} name={name} imageUrl={imageUrl} ingredients={ingredients} price={price}/>
                    )) }
            </div>
        )
    }
...
export default MenuPageItemDirectory;
